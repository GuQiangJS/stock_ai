...
from stock_ai.calcs import cum_return
import numpy as np
...
def xy_split_1(dfs, window, days, x_cols, y_cols=['close'], **kwargs):
    """拆分数据
    ...
    Args:
        ...
        norm_func: 数据构造器。接受参数类型为 :class:`pandas.DataFrame`，返回类型为 :class:`pandas.DataFrame` 的静态方法即可。默认为 :func:`stock_ai.calcs.cum_return`。
        window (int): 窗口期。
        days (int): 结果期。
    ...
    Examples:
        ...
        >>> window, days
        4, 2
        >>> x, y = xy_split_1([pd.DataFrame(arr, columns=['c'])],
    ...
    X = []
    Y = []
    norm_func = kwargs.pop('norm_func', cum_return)
    for df in dfs:
        df_tmp = df.copy()
        if norm_func:
...
class P1(P):
    ...
    def __init__(self, window=3, days=1, x_cols=['close'], y_cols=['close']):
        self.window = window
        self.days = days
        self.x_cols = x_cols
        self.y_cols = y_cols
    ...
    def trans_to_df(self, data, **kwargs):
        ...
        x = []
        df_tmp = data.copy()
        batch_size = self.window + self.days
